...
const jsonName = process.env.MARKET_CONDITION_SMOKE_JSON_NAME || "market-condition-ranking-smoke-report.json";
const markdownName = process.env.MARKET_CONDITION_SMOKE_MARKDOWN_NAME || "market-condition-ranking-smoke-report.md";
const csvName = process.env.MARKET_CONDITION_SMOKE_CSV_NAME || "market-condition-ranking-smoke-report.csv";
const requestTimeoutMs = Number(process.env.MARKET_CONDITION_SMOKE_TIMEOUT_MS || "70000");
const retries = Math.max(1, Number(process.env.MARKET_CONDITION_SMOKE_RETRIES || "4"));
const pauseBetweenQueriesMs = Number(process.env.MARKET_CONDITION_SMOKE_PAUSE_MS || "2000");

const tasks = [
  {
    id: "market_condition_new_base_rent",
  ...
  {
    id: "market_condition_anniversary_date",
...
async function fetchJson(url, payload, label) {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      if (attempt < retries) {
        console.warn(`[market-condition-smoke] ${label} attempt ${attempt}/${retries} failed: ${error instanceof Error ? error.message : String(error)}`);
...
function toMarkdown(report) {
  const lines = [];
  lines.push("# Market Condition Ranking Smoke Report");
  lines.push("");
...
async function main() {
  ...
  for (const task of tasks) {
    try {
      ...
      const expectedCitationFound = top5.some((row) =>
        task.expectedCitations.some((citation) => normalize(row?.citation) === normalize(citation))
      );
  ...
  console.log(`Market condition ranking smoke JSON report written to ${paths.jsonPath}`);
  console.log(`Market condition ranking smoke Markdown report written to ${paths.markdownPath}`);
  console.log(`Market condition ranking smoke CSV report written to ${paths.csvPath}`);
}
