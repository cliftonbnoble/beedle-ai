...
import { buildRetrievalCorpusAdmissionReport, formatRetrievalCorpusAdmissionMarkdown } from "./retrieval-corpus-admission-utils.mjs";

const apiBase = process.env.API_BASE_URL || "http://127.0.0.1:8787";
const idsInput = process.env.RETRIEVAL_DOC_IDS || "";
const limit = Number.parseInt(process.env.RETRIEVAL_DOC_LIMIT || "25", 10);
const realOnly = (process.env.RETRIEVAL_REAL_ONLY || "1") !== "0";
const includeText = (process.env.RETRIEVAL_INCLUDE_TEXT || "0") !== "0";
const reportName = process.env.RETRIEVAL_CORPUS_ADMISSION_REPORT_NAME || "retrieval-corpus-admission-report.json";
const markdownName = process.env.RETRIEVAL_CORPUS_ADMISSION_MARKDOWN_NAME || "retrieval-corpus-admission-report.md";
...
async function main() {
  ...
  await fs.writeFile(mdPath, formatRetrievalCorpusAdmissionMarkdown(report));

  console.log(JSON.stringify(report.summary, null, 2));
  console.log(`Retrieval corpus admission JSON report written to ${jsonPath}`);
  console.log(`Retrieval corpus admission Markdown report written to ${mdPath}`);
}
