...
const manifestInputName = process.env.RETRIEVAL_R47_INPUT_MANIFEST || "retrieval-r46-next-single-manifest.json";
const activationReportName = process.env.RETRIEVAL_R47_ACTIVATION_REPORT_NAME || "retrieval-r47-single-activation-report.json";
const activationMarkdownName = process.env.RETRIEVAL_R47_ACTIVATION_MARKDOWN_NAME || "retrieval-r47-single-activation-report.md";
const liveQaReportName = process.env.RETRIEVAL_R47_LIVE_QA_REPORT_NAME || "retrieval-r47-single-live-qa-report.json";
const liveQaMarkdownName = process.env.RETRIEVAL_R47_LIVE_QA_MARKDOWN_NAME || "retrieval-r47-single-live-qa-report.md";
const rollbackManifestName = process.env.RETRIEVAL_R47_ROLLBACK_MANIFEST_NAME || "retrieval-r47-single-rollback-manifest.json";

const R47_TARGET_DOC_ID = "doc_6497e48b-69b3-44e3-b0c9-e7eb370067e2";
...
function buildActivationMarkdown(report) {
  const lines = [];
  lines.push("# Retrieval R47 Single Activation Report");
  lines.push("");
  lines.push("## Summary");
...
async function main() {
  ...
  if (!r45Repair?.requiredFinalFields?.stateIsSafe || !r45Repair?.requiredFinalFields?.rollbackVerificationPassed) {
    throw new Error("R47 activation aborted: R45 rollback reconciliation is not clean/safe.");
  }
  ...
  if (targetDocId !== R47_TARGET_DOC_ID) {
    throw new Error(`R47 manifest doc mismatch. Expected ${R47_TARGET_DOC_ID}, got ${targetDocId}`);
  }
  ...
  if (!baselineTrustedDocIds.length) throw new Error("No baseline trusted doc IDs in R47 manifest.");
  ...
  console.log(`R47 activation report written to ${activationPath}`);
  console.log(`R47 live QA report written to ${liveQaPath}`);
  console.log(`R47 rollback manifest written to ${rollbackPath}`);
}
