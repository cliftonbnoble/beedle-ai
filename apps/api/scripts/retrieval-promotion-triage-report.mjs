...
import { buildRetrievalPromotionTriageReport, formatRetrievalPromotionTriageMarkdown } from "./retrieval-promotion-triage-utils.mjs";

const apiBase = process.env.API_BASE_URL || "http://127.0.0.1:8787";
const idsInput = process.env.RETRIEVAL_DOC_IDS || "";
const limit = Number.parseInt(process.env.RETRIEVAL_DOC_LIMIT || "200", 10);
const realOnly = (process.env.RETRIEVAL_REAL_ONLY || "1") !== "0";
const includeText = (process.env.RETRIEVAL_INCLUDE_TEXT || "0") !== "0";
const reportName = process.env.RETRIEVAL_PROMOTION_TRIAGE_REPORT_NAME || "retrieval-promotion-triage-report.json";
const markdownName = process.env.RETRIEVAL_PROMOTION_TRIAGE_MARKDOWN_NAME || "retrieval-promotion-triage-report.md";
...
async function main() {
  ...
  await fs.writeFile(mdPath, formatRetrievalPromotionTriageMarkdown(report));

  console.log(JSON.stringify(report.summary, null, 2));
  console.log(`Retrieval promotion triage JSON report written to ${jsonPath}`);
  console.log(`Retrieval promotion triage Markdown report written to ${mdPath}`);
}
