...
import { buildRetrievalReadinessReport, formatRetrievalReadinessMarkdown } from "./retrieval-readiness-report-utils.mjs";
...
const reportName = process.env.RETRIEVAL_READINESS_REPORT_NAME || "retrieval-readiness-report.json";
const markdownName = process.env.RETRIEVAL_READINESS_MARKDOWN_NAME || "retrieval-readiness-report.md";
...
async function main() {
  ...
  await fs.writeFile(mdPath, formatRetrievalReadinessMarkdown(report));

  console.log(JSON.stringify(report.summary, null, 2));
  console.log(`Retrieval readiness JSON report written to ${jsonPath}`);
  console.log(`Retrieval readiness Markdown report written to ${mdPath}`);
}
