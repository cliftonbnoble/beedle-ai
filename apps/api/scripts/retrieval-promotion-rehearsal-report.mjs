...
import { buildRetrievalPromotionRehearsalReport, formatRetrievalPromotionRehearsalMarkdown } from "./retrieval-promotion-rehearsal-utils.mjs";

const apiBase = process.env.API_BASE_URL || "http://127.0.0.1:8787";
const idsInput = process.env.RETRIEVAL_DOC_IDS || "";
const limit = Number.parseInt(process.env.RETRIEVAL_DOC_LIMIT || "250", 10);
const realOnly = (process.env.RETRIEVAL_REAL_ONLY || "1") !== "0";
const includeText = (process.env.RETRIEVAL_INCLUDE_TEXT || "0") !== "0";
const reportName = process.env.RETRIEVAL_PROMOTION_REHEARSAL_REPORT_NAME || "retrieval-promotion-rehearsal-report.json";
const markdownName = process.env.RETRIEVAL_PROMOTION_REHEARSAL_MARKDOWN_NAME || "retrieval-promotion-rehearsal-report.md";
...
async function main() {
  ...
  await fs.writeFile(mdPath, formatRetrievalPromotionRehearsalMarkdown(report));

  console.log(JSON.stringify(report.summary, null, 2));
  console.log(`Retrieval promotion rehearsal JSON report written to ${jsonPath}`);
  console.log(`Retrieval promotion rehearsal Markdown report written to ${mdPath}`);
}
