...
const jsonName = process.env.JUDGE_SOURCE_RECOVERY_JSON_NAME || "judge-name-source-recovery-report.json";
const markdownName = process.env.JUDGE_SOURCE_RECOVERY_MARKDOWN_NAME || "judge-name-source-recovery-report.md";
const csvName = process.env.JUDGE_SOURCE_RECOVERY_CSV_NAME || "judge-name-source-recovery-report.csv";
const apply = (process.env.JUDGE_SOURCE_RECOVERY_APPLY || "0") === "1";
const limit = Number(process.env.JUDGE_SOURCE_RECOVERY_LIMIT || "250");
const busyTimeoutMs = Number(process.env.JUDGE_SOURCE_RECOVERY_BUSY_TIMEOUT_MS || "5000");
const curlTimeoutSeconds = Number(process.env.JUDGE_SOURCE_RECOVERY_CURL_TIMEOUT_SECONDS || "20");
...
async function extractSourceTextWithTextutil(bytes) {
  const tempPath = path.join(os.tmpdir(), `judge-source-${crypto.randomUUID()}.docx`);
...
async function extractRecoverableSourceText(bytes) {
  try {
    ...
    try {
      ...
      // Fall through and rethrow the original parse failure.
    }
    throw primaryError;
  }
}
...
function inferJudgeFromSourceText(text) {
  ...
  if (tailMatches.length === 1) {
    return {
      ...
      evidenceSnippet: normalizeWhitespace(tail.slice(Math.max(0, tail.toLowerCase().indexOf(normalizeJudgeLookupKey(tailMatches[0])) - 120), 400))
    };
  }
  if (tailMatches.length > 1) {
    return {
...
function formatMarkdown(report) {
  const lines = [
    "# Judge Name Source Recovery Report",
    "",
  ...
  for (const row of report.candidateRepairs.slice(0, 50)) {
    lines.push(
      `- \`${row.citation}\` | before=\`${row.currentAuthorName || "<none>"}\` | after=\`${row.nextAuthorName}\` | match=\`${row.matchType}\``
    );
...
async function main() {
  ...
  const rows = await runSqlJson(`
    ...
    WHERE file_type = 'decision_docx'
      ...
      AND (
        author_name IS NULL
        OR trim(author_name) = ''
        OR lower(trim(author_name)) IN ('unknown', 'judge unknown', '<unknown>')
      )
  ...
  for (const row of rows) {
    ...
    const nextAuthorName = inferred.inferredJudgeName || null;
    const shouldUpdate = Boolean(nextAuthorName) && normalizeWhitespace(nextAuthorName) !== normalizeWhitespace(row.currentAuthorName);
    const inspectedRow = {
      ...row,
      nextAuthorName,
    ...
    if (inferred.matchType.startsWith("ambiguous")) {
      ambiguousCandidates.push(inspectedRow);
    }
    if (shouldUpdate) {
      candidateRepairs.push(inspectedRow);
    }
  }
  ...
  const byJudgeMap = new Map();
  for (const row of candidateRepairs) {
    const judgeName = row.nextAuthorName;
    byJudgeMap.set(judgeName, (byJudgeMap.get(judgeName) || 0) + 1);
  }
  ...
  const report = {
    ...
    fetchFailures,
    summary: {
      inspectedDocCount: rows.length,
      fetchedDocCount: rows.length - fetchFailures.length,
      ...
      byJudge: Array.from(byJudgeMap.entries())
  ...
  console.log(`Judge source recovery JSON report written to ${jsonPath}`);
  console.log(`Judge source recovery Markdown report written to ${markdownPath}`);
  console.log(`Judge source recovery CSV report written to ${csvPath}`);
}
