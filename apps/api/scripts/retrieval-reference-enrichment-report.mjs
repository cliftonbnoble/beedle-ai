...
import {
  buildRetrievalReferenceEnrichmentReport,
  formatRetrievalReferenceEnrichmentMarkdown
} from "./retrieval-reference-enrichment-utils.mjs";
...
const reportName = process.env.RETRIEVAL_REFERENCE_ENRICHMENT_REPORT_NAME || "retrieval-reference-enrichment-report.json";
const markdownName =
  process.env.RETRIEVAL_REFERENCE_ENRICHMENT_MARKDOWN_NAME || "retrieval-reference-enrichment-report.md";
...
async function main() {
  ...
  await fs.writeFile(mdPath, formatRetrievalReferenceEnrichmentMarkdown(report));

  console.log(JSON.stringify(report.summary, null, 2));
  console.log(`Retrieval reference enrichment JSON report written to ${jsonPath}`);
  console.log(`Retrieval reference enrichment Markdown report written to ${mdPath}`);
}
