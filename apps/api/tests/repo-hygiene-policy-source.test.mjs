...
test("repo hygiene policy documents report and script cleanup expectations", async () => {
  ...
  assert.match(policy, /apps\/api\/scripts\/archive\//);
  ...
  assert.match(scriptsReadme, /apps\/api\/scripts\/archive\//);
  ...
  assert.match(scriptsReadme, /write:repo-report-cleanup/);
  assert.match(scriptsReadme, /REPO_REPORT_CLEANUP_MAX_APPLY/);
});
