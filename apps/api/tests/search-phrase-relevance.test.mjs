...
test("search scoring uses per-search derived query context in hot row scoring", async () => {
  ...
  assert.match(src, /function literalKeywordTokens\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): string\[\] \{\s*const normalized = precomputed\?\.normalizedQuery \?\? normalize\(query \|\| ""\)[\s\S]*isInfestationAliasQuery\(query, \{ normalizedQuery: normalized \}\)/);
  assert.match(src, /function isLiteralKeywordQuery\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): boolean \{\s*return literalKeywordTokens\(query, precomputed\)\.length > 0/);
  assert.match(src, /function isKeywordFamilyRecallQuery[\s\S]*isLiteralKeywordQuery\(query, \{ normalizedQuery: normalized \}\)[\s\S]*matchedCuratedKeywordFamilies\(query, \{ normalizedQuery: normalized \}\)\.length > 0/);
  ...
  assert.match(src, /const conditionSignals = precomputed\?\.primarySignals \?\? requiredHabitabilityPrimarySignals\(query, \{ normalizedQuery: normalized \}\)/);
  assert.match(src, /function requiredHabitabilityPrimarySignals\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): string\[\] \{\s*return primaryIssueSignals\(query, precomputed\)\.filter/);
  assert.match(src, /function issueQueryIndexCodeHints\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): string\[\] \{\s*const normalized = precomputed\?\.normalizedQuery \?\? normalize\(query \|\| ""\)/);
  assert.match(src, /function issueQueryPhraseHints\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): string\[\] \{\s*const normalized = precomputed\?\.normalizedQuery \?\? normalize\(query \|\| ""\)[\s\S]*inferIssueTerms\(query, normalizedQueryContext\)[\s\S]*issueQueryIndexCodeHints\(query, normalizedQueryContext\)/);
  assert.match(src, /function issueQueryReferenceHints\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): \{ rulesSections: string\[\]; ordinanceSections: string\[\] \} \{\s*const normalized = precomputed\?\.normalizedQuery \?\? normalize\(query \|\| ""\)/);
  assert.match(src, /function lockoutScopePhraseHints\(query: string, precomputed\?: \{ normalizedQuery\?: string \}\): string\[\] \{\s*const normalized = precomputed\?\.normalizedQuery \?\? normalize\(query \|\| ""\)[\s\S]*requiresLockoutSpecificity\(query, normalizedQueryContext\)/);
  assert.match(src, /function habitabilityScopePhraseHints\([\s\S]*precomputed\?: \{ normalizedQuery\?: string; primarySignals\?: string\[\] \}[\s\S]*const normalized = precomputed\?\.normalizedQuery \?\? normalize\(query \|\| ""\)[\s\S]*requiredHabitabilityPrimarySignals\(query, normalizedQueryContext\)/);
  ...
  assert.match(src, /exactMultiWordPhraseScore\(context\.query, searchableText, \{[\s\S]*normalizedGroups: queryDerived\.normalizedPhraseConceptGroups,[\s\S]*normalizedQuery: queryDerived\.normalizedQuery,[\s\S]*normalizedText: loweredSnippet,[\s\S]*phraseTokens: queryDerived\.sentencePhraseOverlapTokens[\s\S]*\}\)/);
