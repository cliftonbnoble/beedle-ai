...
Working the search-focused plan in order. **SEARCH-04 ✅ and SEARCH-05 ✅ are done, verified, and committed. SEARCH-02c is now in progress** — the module split is proceeding bottom-up, one leaf module at a time, each landed only after the golden net stays 27/27 byte-identical (steps 1–2 done: `search-lexical-sql.ts`, `search-text.ts`). **SEARCH-02b stays blocked on 02c** (its predicate surface is extracted as step 4 of the split). A full 10k-line split + ~680-assertion overhaul is deliberately incremental so no step is rushed.
...
| **SEARCH-02c** module split | 🟡 In progress | Split the ~11k-line `search.ts`, migrating the ~680 source-regex assertions module-by-module with the golden net green after each. **Extract bottom-up** (leaves first) so no module imports back from `search.ts` (avoids circular deps). **Steps 1–3 done** (each behavior-neutral: golden 27/27 byte-identical, typecheck clean, test:source 60/60, coupled assertions re-pointed): (1) pure lexical SQL clause/rank builders → `search-lexical-sql.ts`; (2) pure text primitives (`normalize`, `normalizeWhitespace`, `tokenize`, `STOPWORD_TOKENS`, `meaningfulLexicalTokens`, `ftsQuote`, `escapeRegex`, `uniq`, `containsWholeWord`) → `search-text.ts` (−75 lines); (3) pure phrase/concept coverage — 11 fns incl. `phraseConceptGroups`, `phraseConceptVariantsForToken`, `keywordSurfaceVariants`, `phraseSearchFtsQuery`, `phraseConceptCoverage`, `sentencePhraseOverlapScore` → `search-concepts.ts` (−183 lines from `search.ts`; ~12 phrase-suite assertions re-pointed empirically — each regex tested against the file it now lives in; context-coupled `phraseConceptGuardPasses` + `exactMultiWordPhraseScore`, which call not-yet-split classification predicates, deliberately kept in `search.ts`). **Planned order (remaining):** (4) `search-query-classification.ts` (the ~40 `isXxxQuery`/`hasXxxContext` predicates = the 02b surface; depends on text+concepts) → (5) `search-fts.ts` + `search-scope.ts` (depend on text + `env.DB`) → (6) `search-scoring.ts` (scoreRow/finalizeResults) → `search.ts` keeps orchestration only. Each step: extract, import back, migrate that module's assertions, golden-verify. |
