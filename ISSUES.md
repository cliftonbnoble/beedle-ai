...
| **SEARCH-02c** module split | 🟡 In progress | Split the ~11k-line `search.ts`, migrating the ~680 source-regex assertions module-by-module with the golden net green after each. **Extract bottom-up** (leaves first) so no module imports back from `search.ts` (avoids circular deps). **Step 1 done:** the pure lexical SQL clause/rank builders → `search-lexical-sql.ts` (behavior-neutral: golden 27/27 byte-identical, typecheck clean, test:source 60/60, one gate assertion re-pointed). **Planned order:** (2) `search-text.ts` — pure text primitives (`normalize`, `tokenize`, `normalizeWhitespace`, `meaningfulLexicalTokens`, `containsWholeWord`, `escapeRegex`, `ftsQuote`, `uniq`, `STOPWORD_TOKENS`; scattered across ~8 sites, ~40+ call sites + several asserted) → (3) `search-concepts.ts` (phrase/concept coverage, depends on text) → (4) `search-query-classification.ts` (the ~40 `isXxxQuery`/`hasXxxContext` predicates = the 02b surface; depends on text+concepts) → (5) `search-fts.ts` + `search-scope.ts` (depend on text + `env.DB`) → (6) `search-scoring.ts` (scoreRow/finalizeResults) → `search.ts` keeps orchestration only. Each step: extract, import back, migrate that module's assertions, golden-verify. |
