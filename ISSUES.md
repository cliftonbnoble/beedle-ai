...
**At a glance:** 4 High · 16 Medium · 10 Low (30 verified findings). The four Highs are all fast to fix (one is a one-word argument swap). Two themes dominate: (1) the D1 ~100-bound-param limit that SEARCH-05 root-fixed in search still bites the retrieval-activation/rollback path, and (2) error/link plumbing — everything becomes a 400, and the sentinel source-link design keeps producing dead links.
