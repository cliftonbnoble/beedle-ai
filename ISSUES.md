...
| **SEARCH-02b** lexicon extraction | 🟢 Unblocked (02c done) | Fold the 103 `isXxxQuery` / `hasXxxContext` per-topic predicates — now isolated in `search-query-classification.ts` — into a data-driven lexicon (`packages/shared/src/search-concepts.ts` is the seam), one topic family at a time, golden-green after each. The mechanical split (02c) is complete, so this is now a self-contained refactor of one module. |
