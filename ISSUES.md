...
| ABUSE-01 | `244ee4c`, `10b526e` | Added Cloudflare Rate Limiting bindings and fail-closed enforcement before routing costly POSTs: search/debug (60/min), ingestion/vector work (3/min), LLM work (6/min), and destructive admin writes (6/min). The original pass keyed by client IP; AUTH-01 now keys by authenticated username when a session is present, falling back to IP only for unauthenticated public routes. Rejections return `429`/`Retry-After`; unavailable enforcement returns `503` rather than allowing unbounded cost. |
