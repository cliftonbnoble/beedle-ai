...
| SEARCH-06 | `146037f`, pending migration correction | Removed request-time FTS DDL/backfill, eliminating concurrent cold-isolate duplication. Migration `0010_search_fts_backfill.sql` is now a fast compatibility checkpoint: `0008` owns FTS schema/trigger creation, the existing production index is already populated, and triggers maintain future changes. The original full-corpus dedupe/backfill exceeded remote D1 CPU and was intentionally replaced before retrying the protected workflow. |
