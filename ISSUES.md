...
| **SEARCH-02c** module split | 🟡 In progress | Split the ~11k-line `search.ts`, migrating the ~680 source-regex assertions module-by-module with the golden net green after each. **Extract bottom-up** (leaves first) so no module imports back from `search.ts` (avoids circular deps). **Steps 1–2 done** (each behavior-neutral: golden 27/27 byte-identical, typecheck clean, test:source 60/60, coupled assertions re-pointed): (1) pure lexical SQL clause/rank builders → `search-lexical-sql.ts`; (2) pure text primitives (`normalize`, `normalizeWhitespace`, `tokenize`, `STOPWORD_TOKENS`, `meaningfulLexicalTokens`, `ftsQuote`, `escapeRegex`, `uniq`, `containsWholeWord`) → `search-text.ts` (−75 lines from `search.ts`; the 3 moved definition assertions re-pointed across the gate + phrase suites). **Planned order (remaining):** (3) `search-concepts.ts` (phrase/concept coverage, depends on text) → (4) `search-query-classification.ts` (the ~40 `isXxxQuery`/`hasXxxContext` predicates = the 02b surface; depends on text+concepts) → (5) `search-fts.ts` + `search-scope.ts` (depend on text + `env.DB`) → (6) `search-scoring.ts` (scoreRow/finalizeResults) → `search.ts` keeps orchestration only. Each step: extract, import back, migrate that module's assertions, golden-verify. |
