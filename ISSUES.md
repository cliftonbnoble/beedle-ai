...
**Severity:** High  
**Status:** Partially addressed locally: `scoreRow`, layered snippets, document evidence summaries, representative chunk scoring, authority passage scoring, supporting-fact diagnostics, and decision-scope filtering now reuse a per-search derived query context for repeated terms, normalized issue/procedural terms, flags, normalized sentence anchors, normalized secondary tokens, normalized factual metric tokens, normalized phrase concept groups, and judge references; multiple query-specific scoring branches also reuse the pre-normalized row text in the hot scorer. Deeper helper propagation remains open.
...
**Severity:** High  
**Status:** Read-only local baseline added with `report:facet-storage-audit`, which measures current JSON facet coverage, malformed facet JSON, estimated future join-table rows, and top facet values. Schema migration/backfill and query changes remain open.
...
**Severity:** Medium  
**Status:** Partially addressed locally with a repo hygiene policy, API scripts README, `report:repo-scripts` inventory guard, dry-run `report:repo-cleanup-plan` for stale generated reports, and source tests guarding the cleanup policy. These cover report retention, package alias discipline, missing targets, exact duplicate target mappings, command-variant target mappings, unaliased top-level scripts, local report volume, and focused cleanup cadence. Actual script/package alias pruning remains open.
...
- `apps/api/reports` is about `575 MB` by `report:repo-scripts` byte inventory
