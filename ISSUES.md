...
|---|---|---:|---|---|---|
| SEARCH-02 search.ts de-bloat | High | 90% | Hard | High | **02c module split ✅ COMPLETE** — the ~10.9k-line monolith is now 9 focused modules (`search.ts` holds only orchestration, ~1.7k lines, −84%), every extraction golden-verified byte-identical. Remaining: **02b** (fold the now-isolated `search-query-classification` predicates into a data-driven lexicon) — see the Search Sequence section below. |
...
Working the search-focused plan in order. **SEARCH-04 ✅, SEARCH-05 ✅, and SEARCH-02c ✅ are done, verified, and committed.** The module split ran bottom-up over 6 steps (leaf modules → classification → query-analysis → fts → scoring → orchestration), one module at a time, each landed only after the golden net stayed 27/27 byte-identical — the ~10.9k-line monolith is now 9 focused modules and `search.ts` holds only orchestration (~1.7k lines). **SEARCH-02b is now unblocked** — the per-topic predicates are isolated in `search-query-classification.ts`, ready to fold into a data-driven lexicon.
...
| **SEARCH-02b** lexicon extraction | 🟡 In progress | Fold the topic predicates in `search-query-classification.ts` into a data-driven lexicon, one family at a time, golden-green after each. **Family 1 done — the 24 single-pattern predicates** (`isDogQuery`, `isAntQuery`, `isCoolingIssueQuery`, `isSection8Query`, …): their regexes now live in a `QUERY_TOPIC_PATTERNS` / `TEXT_TOPIC_PATTERNS` table and each predicate is a one-line `matchQueryTopic(TABLE.key, …)` wrapper. **Behaviour-identical proven**, not just golden: every one of the 24 regexes is byte-identical to the original, the `!normalized` guard is a verified no-op (each pattern returns false on `""`), and a 384-case old-vs-new battery had 0 mismatches (plus golden 27/27, test:source 60/60). Signatures are preserved so call-site + signature assertions still pass. **Remaining:** the ~77 compound predicates (AND/OR/composed logic, e.g. `isCameraPrivacyQuery` = camera-terms ∧ privacy-terms) are genuinely bespoke, not near-identical — a later family can add `anyOf`/`allOf` lexicon shapes for the pure-OR / pure-AND ones, but mixed `(A∧B)∨C` and predicate-composed ones stay explicit. |
