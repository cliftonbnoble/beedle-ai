...
**Severity:** High  
**Status:** Partially addressed locally: `scoreRow` now receives a per-search derived query context for repeated terms, flags, and judge references, and multiple query-specific scoring branches reuse the pre-normalized row text in the hot scorer. Deeper helper propagation remains open.
