...
| **SEARCH-02c** module split | 🟡 In progress | Split the ~11k-line `search.ts` along seams (query-classification · scope/SQL · lexical/FTS · vector · decision-layer/ranking · snippet), migrating the ~680 source-regex assertions module-by-module with the golden net green after each. **Step 1 done:** the pure lexical SQL clause/rank builders (`buildLexicalMatchClause`/`RankExpr`, whole-word variants, `normalizedWholeWordExpr`) extracted to `search-lexical-sql.ts` — behavior-neutral (golden 27/27 byte-identical, typecheck clean, test:source 60/60, one gate assertion re-pointed at the module). Establishes the extraction pattern for the larger clusters. |
