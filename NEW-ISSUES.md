...
### NS-27 · Vector failures are invisible — and the error fallback drops the namespace — ◐ PARTIAL (21ea794)
**Status:** bare-token vector-first queries (harassment/buyout/capital improvement) now lexically rescue when the vector channel yields nothing — 0 results → p5 1.00 in ~0.5-1s (bare_harassment eval entry). Multi-token forms stay empty pending NS-17 (their rescue rows are collapsed by vector-score guard thresholds unmeetable with a dead vector channel, behind a 16-60s un-instrumented crawl — instrument + fix with the guard rework). Underlying error-visibility work (surfacing vector failures in diagnostics) still open.
...
### NS-34 · Two documents sharing one citation string render as duplicate result rows (discovered during NS-03)
