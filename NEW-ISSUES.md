...
### NS-10 · Vector recall is a 25-chunk afterthought, and "vector-first" topics can't reach it — ✅ RESOLVED in code (dac12ff); prod A/B pending
**Status:** vector-first check hoisted above the ≤2-token skip; topK ceiling 25→100 (metadata no longer requested — merge reads id+score only), floor 40; vectorSearchLimit floors at 50 decoupled from page size. Locally inert/byte-stable.
...
### NS-11 · The keyword-family + judge "universe" is the 200 newest docs, not the 200 best — ✅ RESOLVED (3cf66a1)
**Status:** the universe now qualifies documents by an FTS match on the execution terms before the recency-ordered cap (fetchFtsMatchingDocumentIds), falling back to the recency universe when FTS is unavailable. D1-adjudicated golden movement: infestation+judge strictly better (24-chunk doc replaces 15), mold+judge lateral tie (23 vs 24) re-pinned with rationale.
...
### NS-16 · Fusion is incoherent: SQL rank weights never reach final scoring; vector is a near-binary +0.15 — ◐ VECTOR TERM RESOLVED (741dfee); field-aware half open
**Status:** calibratedVectorFusionScore spreads the bge 0.55-0.95 band across [0,1] before the 0.23 weight — fixed affine (pagination-stable), fusion-term-only (guard thresholds keep raw cosine). Locally byte-stable; prod effect ships in the remote A/B batch with NS-22. The SQL-rank/scoreRow field-awareness disagreement remains open (larger, separately gated).
...
### NS-19 · Decision-scope slice (8–14 docs) happens BEFORE the layer boosts that would reorder it — ✅ RESOLVED (abf4215)
**Status:** scope admits 3× the limit so the ±0.16-0.42 layer boosts can reorder; exact_phrase keeps the tight scope (widening measurably let topical docs displace phrase-CONTAINING docs — eval-caught, quoted p5 1.00→0.20, refined, restored).
...
### NS-21 · Vector topK is tied to pageWindow (~30 chunks corpus-wide) and skipped when FTS "has enough" — ◐ CAPS RESOLVED (dac12ff); skip-vs-downweight deferred
**Status:** topK/limit floors landed (see NS-10). The phraseFtsHasEnoughEvidence vector skip is deliberately untouched: turning it into a downweight is a tuning judgment that needs remote measurement (it also saves an embed round-trip per well-served phrase query).
